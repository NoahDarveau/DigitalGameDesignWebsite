...
"A pleasant glow.", "But alas, all things fade.", "Just like this light", "That beckons me onward...", "In an everlong pursuit of the radiance", "That illuminates this world...",
"And guides me.", "Alas, my time here draws near", "Like this light of mine...", "I too, fade.", "With a final breath..."];
	...
	for (let dy = -lightRadius; dy <= lightRadius; dy++) {
		for (let dx = -lightRadius; dx <= lightRadius; dx++) {
			...
			if ((dy !== 0 || dx !== 0) && (coords.x >= 0 && coords.x < PS.gridSize().width && coords.y >= 0 && coords.y < PS.gridSize().height)
				...
				if (PS.data(coords.x, coords.y) === "POWERUP") {
...
let checkLocation = function () {
	...
	else if (PS.data(PS.spriteMove(playerSprite).x, PS.spriteMove(playerSprite).y) === "POWERUP") {
		//lightRadius++;
		phraseNum++;
		PS.statusText(phrases[phraseNum]);
		darknessFactor = Math.sqrt(darknessFactor);
