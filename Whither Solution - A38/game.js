...
const W = ( function () {
	...
	let _COLOR_ACTOR_BORDER = PS.COLOR_GRAY;
	...
	let colors = [];
	colors = PS.unmakeRGB(_COLOR_FLOOR, colors);
	let borderFadeInc =  colors[0] / 10;
	let moveCount = 0;
	...
	const move = function ( x, y ) {
		...
		if (_level === 20) {
			PS.unmakeRGB(PS.gridColor(), colors);
			PS.gridColor(colors[0] - borderFadeInc, colors[1] - borderFadeInc, colors[2] - borderFadeInc);
			PS.unmakeRGB(_COLOR_FLOOR, colors);
			_COLOR_FLOOR = PS.makeRGB(colors[0] - borderFadeInc,colors[1] - borderFadeInc, colors[2] - borderFadeInc);
			_COLOR_ACTOR_BORDER = _COLOR_FLOOR;
			PS.color(PS.ALL, PS.ALL, _COLOR_FLOOR);
			PS.bgColor(PS.ALL, PS.ALL, _COLOR_FLOOR);
			PS.borderColor(PS.ALL, PS.ALL, _COLOR_FLOOR);
			PS.borderColor(_goal_x, _goal_y, PS.COLOR_BLACK);
			let volume = 1 - (moveCount * 0.1);
			if (moveCount === 9) {
				PS.audioFade(_channel, PS.CURRENT, volume, 1000);
			}
			else {
				PS.audioFade(_channel, PS.CURRENT, volume, 100);
			}
			moveCount++;
		}
