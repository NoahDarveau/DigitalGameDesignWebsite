...
let updateLightingRayTracing = function() {
	PS.color(PS.ALL, PS.ALL, PS.COLOR_BLACK);
	...
	for (let dy = -lightRadius; dy <= lightRadius; dy++) {
		...
		for (let dx = -lightRadius; dx <= lightRadius; dx++) {
			...
			//PS.debug("dy: " + dy + '\n');
			let ray = PS.line(PS.spriteMove(playerSprite).x, PS.spriteMove(playerSprite).y, PS.spriteMove(playerSprite).x + dx, PS.spriteMove(playerSprite).y + dy);

			//PS.debug(ray);

			for (let i = 0; i < ray.length; i++) {

				//PS.debug("index: "+ray.length+", Ray x: "+ray[i][0] + ", Ray y: "+ray[i][1]+'\n');
				if ((dy !== 0 || dx !== 0) && (ray[i][0] >= 0 && ray[i][0] < PS.gridSize().width && ray[i][1] >= 0 && ray[i][1] < PS.gridSize().height)
					...
					if (PS.data(ray[i][0], ray[i][1]) === "POWERUP") { //if a powerup is visible, draw the powerup
						PS.color(ray[i][0], ray[i][1], 12 * lightLevel, 12 * lightLevel, 12 * lightLevel);
					}
